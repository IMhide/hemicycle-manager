...
<div class="space-y-4 text-sm">
	{#if showSexe}
		<div>
			<div class="text-xs uppercase tracking-widest text-fg-muted mb-1.5">Sexe</div>
	...
	{#if familles.length > 0}
		<div>
			<div class="text-xs uppercase tracking-widest text-fg-muted mb-1.5">
				Familles politiques
				{#if famillesSelected.length > 0}
					<span class="text-link">({famillesSelected.length})</span>
				{/if}
			</div>
				...
				{#each familles as f (f.id)}
					...
					<button
						class="text-xs text-left px-2 py-1 rounded {checked
							? 'bg-accent/15 border border-accent/60 text-fg'
							: 'border border-border-soft text-fg-muted hover:text-fg'}"
		...
		<div>
			<div class="text-xs uppercase tracking-widest text-fg-muted mb-1.5">
				...
				{#if badgesSelected.length > 0}
					<span class="text-link">({badgesSelected.length})</span>
				{/if}
			</div>

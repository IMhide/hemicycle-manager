<script lang="ts">
	...
	let { title, children, size = 'sm' }: Props = $props();
...
<span class="relative inline-flex items-center align-middle">
	...
	{#if open}
		<span
			class="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 rounded-lg bg-slate-900 border border-assembly-border shadow-xl text-xs leading-relaxed text-slate-200 normal-case font-normal tracking-normal"
			role="tooltip"
		>
			{#if title}
			...
			{@render children()}
			<span
				class="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-slate-900 border-r border-b border-assembly-border rotate-45 -mt-1"
			></span>
		</span>
	{/if}
</span>

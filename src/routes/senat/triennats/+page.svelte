	...
	<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
		{#each triennatsSorted as t (t.id)}
			<a
				...
				class="card p-4 hover:border-accent/60 transition-colors flex flex-col gap-2"
			>
				...
				<div class="grid grid-cols-3 gap-2 mt-1 text-center">
					<div>
						<div class="text-[9px] uppercase text-fg-muted">Sessions</div>
						<div class="title-display text-lg tabular-nums">{t.sessions.length}</div>
					</div>
					<div>
						<div class="text-[9px] uppercase text-fg-muted">Sénateurs</div>
						<div class="title-display text-lg tabular-nums">{t.nbSenateursActifs}</div>
					</div>
					<div>
						<div class="text-[9px] uppercase text-fg-muted">Scrutins</div>
